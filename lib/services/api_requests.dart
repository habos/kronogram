...
class APIcalls extends APIrequests {
  Future<List<KronoInstaPost>> requestInstaPosts(var userInfo) async {
    ...
    final graphResponse = await http.get('https://graph.instagram.com/me?fields=media{id,caption,media_type,media_url,timestamp,children{media_type,media_url}}&access_token=$token');
  ...
  Future<List<KronoFacebookPost>> requestFbPosts(var userInfo) async {
    ...
    final token = userInfo['token'];
    final graphResponse = await http.get('https://graph.facebook.com/me/?fields=posts{id,created_time,name,message,place,attachments{media{image},type,subattachments}}&access_token=$token');
    final jsonPostHistory = JSON.jsonDecode(graphResponse.body);
